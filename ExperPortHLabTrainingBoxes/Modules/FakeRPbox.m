function out=FakeRPbox(varargin)
global exper
...
switch action
    ...
    case 'init'
        ...
        InitParam(me,'RP_AO_Out','value',[0 0]);
        InitSoftTrg;
        InitRP;
        InitAO;
        InitBits; n=n+1.5;
...
%         SaveParamsTrial(me);
    ...
    case 'runrpx'
        rpTimer=GetParam(me,'rpTimer');
        if GetParam(me,'RunRPx')
            SetParamUI(me,'RunRPx','backgroundcolor',[0.9 0 0],'string','Running...');
            UpdatePeriod=GetParam(me,'UpdatePeriod')/1000; % update period in sec
    ...
    case 'ao_out'
        ...
        RP_AO_Out=GetParam(me,'RP_AO_Out');
        for i=1:length(val)
            if RP_AO_Out(AO(i))>=0
                %change the color of the button 
                if val(i)
...
M=zeros(36,10); % 36 states and 7 input,1 timer, 2 output
[rows columns]=size(m);
