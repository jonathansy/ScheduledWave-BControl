...
switch action
    ...
    case 'init'
        ...
        disp('rpbox initialized'); %Added by AS 6/9/14
        ...
        %% THIS USED TO BE 350 'value' (SP):
        InitParam(me,'UpdatePeriod','ui','edit','value',100,'pref',0,'pos',[h n*vs hs*.7 vs]); n=n+1;
    ...
    case 'update'
        if existparam(me, 'RP')
            ...
            % If pole is in reach and we are to move it, do it
 %           if (State == 41)
        %% HOW DO I ACCESS THIS?? UGH!
  %              disp('TOUCH IT !!!');
  %            SavingSection
 %exper.pole_detect_sp.param.object.value % ITS HERE
 %protocols=GetParam('SavingSection','MouseName','value')
 %           end
            ...
            if localFoundState35Flag
                if localProcessingState35==0, 
                    SetParam(me, 'ProcessingState35', 1); 
                    localProcessingState35 = 1;
                    rpbox('state35'); 
                else
                end;
            elseif State == 0
...
%         SaveParamsTrial(me);
    ...
    case 'runrpx'
        if GetParam(me,'RunRPx') & GetParam(me, 'RunStartEnabled')
            SetParamUI(me,'RunRPx','backgroundcolor',[0.9 0 0],'string','Running...'); % RGB: (.9 0 0 ), just tmp
    ...
    case 'manual_rpbox_timer',
     ...
     while( GetParam(me, 'Run') ),
        if ismember(fake_rp_box, [3 4]), % We drive state machine AND 'update'
           ...
           if elapsed > GetParam('rpbox', 'UpdatePeriod')/1000,
              SetParam(me,'last_update_time',clock); rpbox('update');
           end;
   ...
   case 'getsoundmachine'
        ...
        out = invokeWrapper(RPS, 'GetMachine');

   case 'getstatemachine'
        RP = GetParam(me, 'RP');
        out = invokeWrapper(RP, 'GetMachine');        
   ...
   case 'get_state_matrix_nrows',
     global fake_rp_box;
     if fake_rp_box ==0, out = 512; else out = 2048; end;
...
nrows = rpbox('get_state_matrix_nrows');

%DHO_07may07: Comment out following, which forces 10 col matrix.  Don't need.
M = m;
%{ 
% fill out the full state matrix into 36 rows
M=zeros(nrows,10); % nrows states and 7 input,1 timer, 2 output
[rows columns]=size(m);
...
global fake_rp_box; %DHO_07may07
if fake_rp_box == 0 %DHO_07may07: added if statement bc state_to_state_addr breaks w/ <10 col state matrix.
...
% if invokeWrapper(RPS,'ConnectRM1','USB',1)   % DHO, using RP2 FSM and RM1 sound machine %connect to RM1 using 'USB' method
...
RPSS=GetParam('rpbox', 'RPStereoSound');
if strcmp(GetParam('rpbox','RPSSDevice'),'RP2')
  warning('I have no idea how to set the sample rate for an RP2!');
  return;
else
  invokeWrapper(RPSS, 'SetSampleRate', samplerate);
end;
