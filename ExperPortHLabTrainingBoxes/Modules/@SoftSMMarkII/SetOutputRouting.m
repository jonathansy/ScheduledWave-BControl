...
%                          the state matrix.
function [sm] = SetOutputRouting(sm, orouting)
    
    sm = get(sm.Fig, 'UserData');

    if (~isa(orouting, 'cell')),
        error('Output routng is an Mx1 cell array of structs');
    end;
    for i=1:length(orouting),
        s = orouting{i};
        if (~isa(s, 'struct') | ~isfield(s, 'type') | ~isfield(s, 'data')),
            error('Output routing should contain only structs with the fields ''type'' and ''data''');
        end;
        switch s.type
            case 'dout'
                if (s.data ~= '0-15'),
                    error('For now, SoftSMMarkII only supports ''dout'' output routing on channels 0-15');
                end;
            case 'trig'
                error('For now, SoftSMMarkII does not support trigger outputs.');
            case {'udp', 'tcp', 'sched_wave', 'noop', 'sound' }
                % ok
            otherwise
                error(sprintf('Unknown output routing type %s', s.type))
        end;
    end;
    sm.OutputRouting = orouting;
