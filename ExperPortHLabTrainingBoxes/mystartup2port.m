...
if ismember(hostname, {'kaikai1'}), %%{'kaikai1' 'hnb228-labpc5'}),
    fake_rp_box = 3;
elseif ismember(hostname, {'s7255'}),%'hnb228-labpc5' 'd5xr7xh1' 'labadmin-pc' 'oconnord-ww1' 'computer-labx' 'svobodalab-2b' 'svobodalab-2a' 'svobodalab-2i' 'svobodalab-2k'}),
    fake_rp_box = 2;
else
...
switch hostname
    ...
    case 's7255'
        ...
        state_machine_properties.server = '68.181.114.126';%% Edited for Hires RTL2 RTLinux box 10/1/14
        %state_machine_properties.server = '10.102.32.36';
      %  state_machine_properties.server = '10.102.30.56';
        state_machine_properties.sm_number = 2;
        state_machine_properties.output_routing = {struct('type', 'dout','data', '8-15'); struct('type', 'noop','data', '7')}; % Breakout box ports 1-4, '0-9' changed from '10-19' by JS
        %state_machine_properties.input_events = [5 -5 ]; % Get beam break input from breakout box port 1
        state_machine_properties.input_events = [6 -6 5 -5]; % Get beam break input from breakout box port 1
