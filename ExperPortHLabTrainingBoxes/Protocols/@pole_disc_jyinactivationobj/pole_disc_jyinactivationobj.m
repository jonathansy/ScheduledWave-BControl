function [obj] = stim_timingobj(name)
...
set(value(myfig), 'Position', [604   267   648   715]);
...
% History of response. 1 for lick, 0 for no-lick:
...
% Items retrieved from the EmbCFSM log:
SoloParamHandle(obj, 'embc_log', 'value', {});
...
DeclareGlobals(obj, 'ro_args', {'n_done_trials', 'n_started_trials', ...
                    'response_history', 'pole_position_list','embc_log'});

% Let RewardsSection, the part that parses what happened at the end of
% a trial, write to response_history:
SoloFunctionAddVars('RewardsSection', 'rw_args', 'response_history');
...
% Section that chooses the next trial type and keeps track of previous
% trial type history. 
[x, y] = TrialTypeSection(obj, 'init', x, y);

figpos = get(gcf, 'Position');
HeaderParam(obj, 'prot_title', 'Whisker Discrimination + Stimulation', ...
            x, y, 'position', [100 figpos(4)-25, 400 20]);
...
% 1) parse the result of the latest trial, update response_history
