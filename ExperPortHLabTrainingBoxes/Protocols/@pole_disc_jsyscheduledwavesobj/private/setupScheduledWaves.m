...
% TODO - Put these IDs in a Solo global variable - TODO
%WARNING: DO NOT USE WAVE 3 OR 4 AS WAVE ID VALUES. DIO WILL TRIGGER...
%... ACTIVATION. MUCH BAD.      

%Analog scheduled waves: Galvos
wave_id_x_galvo = 10;               AO_chan_x_galvo = 1;
wave_id_y_galvo = 11;               AO_chan_y_galvo = 2;

%Digital scheduled waves: Lasers, masking
%TODO: make the AOM and masking flashes digital. Change indices for...
%...channels. 
wave_id_aom_473 = 12;               AO_chan_aom_473 = 1;
%wave_id_aom_594 = 13;               AO_chan_aom_594 = 6;  - For stim
wave_id_masking_flash_blue = 14;    AO_chan_masking_flash_blue = 2;
%wave_id_masking_flash_orange = 15;  AO_chan_masking_flash_blue = 2;
wave_id_shutter = 15;               DIO_chan_shutter = 3;
...
schedWv_Shutter1 = [wave_id_shutter -1 -1 DIO_chan_shutter 0 0 shutterDurationInSec 0 DIO_shutter_loop]; %old value [wave_id_shutter -1 -1 DIO_chan_shutter 0 0 shutterDurationInSec 0 DIO_shutter_loop]
...
if galvo==1
    ...
    %------------------------
    % Make y-galvo waveform:
    fullwv = repmat(-.25, [1 delay+stimdur*task_periods_per_ms])% zeros([1 delay+stimdur*task_periods_per_ms]); % 2 sec
    schedWv_YGalvo1 = [fullwv; zeros([1 numel(fullwv)])];
end;
...
machine = SetScheduledWaves(machine, wave_id_x_galvo, AO_chan_x_galvo, AOLoop, schedWv_XGalvo1); % ORIGINAL VALUES: wave_id_x_galvo = 4, AO_chan_x_galvo = 0; 
