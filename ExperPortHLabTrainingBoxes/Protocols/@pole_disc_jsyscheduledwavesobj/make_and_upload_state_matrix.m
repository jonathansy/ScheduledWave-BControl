...
%                    '5ms_pulse_50Hz', x, y);
        ...
        %disp(sprintf('session type: %s',SessionTypeSection(obj,'get_session_type')))
        ...
        switch SessionTypeSection(obj,'get_session_type')
            ...
            case 'Laser-473nm-On-Min'
                ...
                stm = [stm; zeros(512-rows(stm),10)]; state_matrix.value = stm;
                stm = [stm(:,1:2),stm(:,7:10)]; % column removal is a temporary fix 
                stm(:,7) = 0;
            ...
            case 'Discrim'
                ...
                stim_delay_in_task_periods = stim_delay_in_ms*task_periods_per_ms; % task_periods_per_ms defined in setupScheduledWaves
                stim_dur_in_task_periods = ceil(dur*task_periods_per_ms); % Can't have fractional task periods.
%                stim_dur_in_task_periods = ceil(stim_dur_in_ms*task_periods_per_ms); % Can't have
%                fractional task periods.  OLD
                disp('stim delay')
                disp(stim_delay_in_task_periods);
                disp('stim_dur')
                disp(stim_dur_in_task_periods);
...
%                    value(WhiskerThreshLow),value(WhiskerThreshHigh)); % re-write with inputParser
                ...
                WhiskerThreshLow = 0; % TODO: Dummy values. To be replaced for functional use. 
                WhiskerThreshHigh = 1;
                G = preprocessEmbC(G,stim_trial,stim_delay_in_task_periods,stim_dur_in_task_periods,...
               ...
               % SetStateProgram(machine,'matrix',stm);
                SetStateProgram(machine,'matrix',stm,'globals',G,'tickfunc','tick_func','threshfunc','thresh_func',...
                    'entryfuncs',{40,'start_trial_func'},'initfunc','init_func','fsm_swap_on_state_0',0);
