...
/*%WARNING: DO NOT USE 3 OR 4 AS WAVE ID VALUES. DIO FROM ANY CHANNEL WILL TRIGGER ACTIVATION. */
struct wave_id_list wave_ids = {.x_galvo = 10, .y_galvo = 11, .aom_473 = 12, .aom_594 = 13, 
.masking_flash_blue = 14, .masking_flash_orange = 15, .shutter = 16};

const unsigned whisker_detector_ai_chan = 5; /* Analog input channel for whisker position readings. */
const unsigned aom_473_chan = 1; /* Analog output channel for AOM for 473 nm laser. */
const unsigned aom_594_chan = 1; /* Analog output channel for AOM for 594 nm laser. */
...
void tick_func(void)
{
    ...
    if (stim_state[0] == 41) {   /* (stim_state[0] == 0) {  -- inactivation during whisker contact only*/
        
        
        stim_delay = sizeof(stim_queue) / sizeof(unsigned) - 1;
        
        /*v = readAI(whisker_detector_ai_chan);
         v assignment is hard coded until RTWT works
         */
        v = 4.0;
 ...
 /*   unsigned num_log_items;
     ...
     *it in the queue. */
        ...
        v_state_last = 0; /* v_state_last = v_state*/
        
        
    }  else if (stim_state[0] == 1) {   /* stim_state[0] == 1  inactivation during sample or delay period*/
