...
%    swid=2^1;
   ...
   switch SessionTypeSection(obj,'get_session_type')
      ...
      case 'Water-Valve-Calibration'
             ...
             ndrops = 100; delay = 1;
             openvalve = [b+1 b+1 b+1 b+1 b+2 wvtm wvid 0]; 
             closevalve = [b+1 b+1 b+1 b+1 b+2 delay 0 0];
             onecycle = [openvalve; closevalve];
             m = repmat(onecycle, ndrops, 1);    
             x = [repmat((0:(2*ndrops-1))',1,5) zeros(2*ndrops,3)];
             m = m+x; m = [b+1 b b b 35 999 0 0; m];
...
%                  ];
       ...
       case 'Beam-Break-Indicator'
             stm = [stm ; 
                 b+1   b  b+1  b      35  999  0      0  ; ... 
                 b+1   b  b+1  b    35  999  LEDid  0  ; ...   
                 ];
...
%             trialnum = 511; %63;
            ...
            m(end,5) = b+1; % jump back to state that triggers pole descent.
